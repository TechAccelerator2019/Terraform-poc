...
func TestVersionListing(t *testing.T) {
	...
	allVersions, err := i.listProviderVersions(addrs.NewDefaultProviderConfig("test"))
...
func TestProviderInstallerGet(t *testing.T) {
	...
	_, _, err = i.Get(addrs.NewDefaultProviderConfig("test"), AllVersions)
	...
	{
		_, _, err := i.Get(addrs.NewDefaultProviderConfig("test"), ConstraintStr(">9.0.0").MustParse())
	...
	{
		pc := addrs.NewDefaultProviderConfig("nonexist")
		_, _, err := i.Get(pc, AllVersions)
		if err != ErrorNoSuchProvider {
			t.Fatal("want error for no such provider")
		}
	}

	gotMeta, _, err := i.Get(addrs.NewDefaultProviderConfig("test"), AllVersions)
	if err != nil {
		t.Fatal(err)
	}
